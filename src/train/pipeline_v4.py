...
def pipeline(
    ...
    X_train = X_train.numpy().reshape(-1, seq_len * dim_in)
    y_train = y_train.numpy()

    X_val = X_val.numpy().reshape(-1, seq_len * dim_in)
    y_val = y_val.numpy()
    ...
    mfh_base = xgb.XGBRegressor(device="cuda")
    mfh = MultiOutputRegressor(mfh_base, n_jobs=-1)
    ...
    model_base = xgb.XGBRegressor(device="cuda")
    model = MultiOutputRegressor(model_base, n_jobs=-1)

    if isinstance(model(), nn.Module):
        raise ValueError("torch Model not supported yet - I will implement it later")
